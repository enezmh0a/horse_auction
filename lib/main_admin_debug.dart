...
Future<(bool isAdmin, Map<String, dynamic>? claims, String uid, String tokenDebug)> refreshAdminClaim() async {
  final auth = FirebaseAuth.instance;
  ...
  final snippet = 'sub: ${token.claims?['sub'] ?? user.uid}, '
      'firebase: ${token.claims?['firebase'] ?? {}}, '
      'sign_in_provider: ${(token.claims?['firebase'] as Map?)?['sign_in_provider'] ?? 'unknown'}';

  return (isAdmin, (token.claims ?? {}) as Map<String, dynamic>?, user.uid, snippet);
}
...
class _AdminHomeState extends State<AdminHome> {
  ...
  final FirebaseFunctions _f = FirebaseFunctions.instanceFor(region: 'me-central1');
        ...
        child: ListView(
          children: [
            ...
            Row(
              children: [
                Expanded(
                  child: SelectableText('UID: $_uid', style: infoStyle),
                ),
                IconButton(
                  tooltip: 'Copy UID',
            ...
            Wrap(
              spacing: 12,
              children: [
                FilledButton(onPressed: _isAdmin ? _accept : null, child: const Text('Accept')),
                OutlinedButton(onPressed: _isAdmin ? _reject : null, child: const Text('Reject')),
              ],
            ),
